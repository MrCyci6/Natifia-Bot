	...
	async execute(client, interaction, color) {
        ...
        try {
    		...
    		const embed = new Discord.EmbedBuilder()
    			...
    			.setFooter({text: `© ${client.config.server.year} - ${client.config.server.name}`})
    			.setColor(color)
    			.setDescription(list.slice(p0, p1).join("\n"))
                .setFooter({text: `© ${client.config.server.year} - ${client.config.server.name} » Total: ${invitations.length} | by ${client.author}`, iconURL: client.user.displayAvatarURL({dynamic: true})})
