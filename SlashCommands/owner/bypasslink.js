					...
					if(choice == "member") {
						...
						try {
			                ...
			                const embed = new Discord.EmbedBuilder()
			                    ...
			                    .setFooter({text: `© ${client.config.server.year} - ${client.config.server.name} » Total: ${wlm.length} | by ${client.author}`, iconURL: client.user.displayAvatarURL({dynamic: true})})
					...
					if(choice == "channel") {
						...
						try {
			                ...
			                const embed = new Discord.EmbedBuilder()
			                    ...
			                    .setColor(color)
			                    .setDescription(list.slice(p0, p1).join("\n"))
			                    .setFooter({text: `© ${client.config.server.year} - ${client.config.server.name} » Total: ${wl.length} | by ${client.author}`, iconURL: client.user.displayAvatarURL({dynamic: true})})
